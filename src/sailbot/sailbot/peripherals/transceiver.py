...
class Transceiver:
    ...
    def send_heartbeat(self):
        ...
        """
        data_str = f"DATA: {self.gps.latitude},{self.gps.latitude},{self.rudder_angle},{self.sail_angle}," \
            ...
            for msg in msgs:
                ...
                if len(ary) > 0:
                    ...
                    if ary[0] == 'sail' or ary[0] == "S":
                        ...
                        if self.manualControl:
                            rudderMidPoint = (float(c.config['CONSTANTS']["rudder_angle_max"]) - float(
                                c.config['CONSTANTS']['rudder_angle_min'])) / 2
                            ...
                            if float(ary[1]) < rudderMidPoint + halfDeadZone and float(
                                    ary[1]) > rudderMidPoint - halfDeadZone:
                                self.targetRudder = float(
                                    rudderMidPoint) - 45  # values read in range from 0:90 instead of -45:45
                            else:
                                self.targetRudder = float(
                                    ary[1]) - 45  # values read in range from 0:90 instead of -45:45
                        ...
                        0] == 'mode':  # set boats mode to the value specified, command should be formatted : "mode {value}" value is int 1-5
                        try:
