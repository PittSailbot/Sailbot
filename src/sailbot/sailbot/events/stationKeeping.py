...
class Station_Keeping(Event):
    ...
    def SK_perc_guide(self, inp_arr, type_arr, buoy_arr):
        ...
        a = [
            ...
            2,
            6,
        ]  # optimizing code with for rather then long ass list
        # 0,1, 2,3, 4,5, 6,7
        for i in range(4):  # 0,1,2,3
            ...
            mid_arr.append(p)  # p
            mid_arr.append(
                self.SK_f(p, buoy_arr[j1], buoy_arr[k1], buoy_arr[j2], buoy_arr[k2])
            )  # p,j1,k1,j2,k2
        ...
        m2 = self.SK_m(
            mid_arr[2], mid_arr[3], mid_arr[6], mid_arr[7]
        )  # slope between side line's midpoints
        ...
        # front
        ret_arr.append(
            self.SK_m(buoy_arr[0], buoy_arr[1], buoy_arr[2], buoy_arr[3])
        )  # m buoy1,buoy2
        ret_arr.append(
            self.SK_v(buoy_arr[0], buoy_arr[1], buoy_arr[2], buoy_arr[3])
        )  # b buoy1,buoy2
        # left
        ret_arr.append(
            self.SK_m(buoy_arr[0], buoy_arr[1], buoy_arr[4], buoy_arr[5])
        )  # m buoy1,buoy3
        ret_arr.append(
            self.SK_v(buoy_arr[0], buoy_arr[1], buoy_arr[4], buoy_arr[5])
        )  # b buoy1,buoy3
        # right
        ret_arr.append(
            self.SK_v(buoy_arr[2], buoy_arr[3], buoy_arr[6], buoy_arr[7])
        )  # m buoy2,buoy4
        ret_arr.append(
            self.SK_v(buoy_arr[2], buoy_arr[3], buoy_arr[6], buoy_arr[7])
        )  # b buoy2,buoy4
        ...
        # back
        ret_arr.append(
            self.SK_m(buoy_arr[4], buoy_arr[5], buoy_arr[6], buoy_arr[7])
        )  # m buoy3,buoy4
        ret_arr.append(
            self.SK_v(buoy_arr[4], buoy_arr[5], buoy_arr[6], buoy_arr[7])
        )  # b buoy3,buoy4
    ...
    def SK_line_check(self, Tarr, Barr, mid_m):
        ...
        if abs(mid_m) < 1:  # Barr is secretly the mid m line shhhhhhh (LOOK AT ME)
            # sideways  -------------------
            # x=(y-b)/m
            Fa = (self.gps.latitude - Tarr[1]) / Tarr[0]
            Fb = self.gps.longitude
            Fc = (self.gps.latitude - Barr[1]) / Barr[0]
        else:
            # rightways  -------------------
            # y=mx+b
            Fa = Tarr[0] * self.gps.longitude + Tarr[1]
            Fb = self.gps.latitude
            Fc = Barr[0] * self.gps.longitude + Barr[1]
