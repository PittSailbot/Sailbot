...
from sailbot.utils.utils import CameraServoState, Waypoint, has_reached_waypoint
...
class DummyEventSearch(Event):
    ...
    def __init__(self, event_info):
        ...
        self.gps_sub = self.create_subscription(String, "s/GPS", self.gps_callback, 1)
    ...
    def next_gps(self):
        self.cameraLoc = (self.cameraLoc + 5) % 90

        pitch = 0
        yaw = self.cameraLoc
        msg = CameraServoState(yaw, pitch).toRosMessage()
        self.camera_servo_pub.publish(msg)
