...
void setup() {
  ...
  // use SPI library to assign RX on pico as MISO (Master-In, Slave_Out). We don't need to assign
  // the MOSI (Master-Out, Slave-In) pin because the pico doesnt need to send anything back to the
  // sensor
...
void loop() {
  // begin the transaction with the previous settings, and set the slave select pin (csPin) to LOW
